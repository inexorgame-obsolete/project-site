...
class Comments {
	 ...
	 */
	public function __construct($module, $identifier)
	{
		$this->_module     = $module;
	 ...
	 */
	public function getComments($order = "DESC", $answerTo = null, $limit = array(30, 5, 2), $offset = array(0, 0, 0))
	{
		if(!isset($offset[0])) $offset[0] = 0;
		$comments = $this->_CI->comments_model->get_comments($this->_module, $this->_identifier, $order, $limit[0], $offset[0]);
		array_unshift($limit);
		array_unshift($offset);
		if(count($limit) > 0)
		{
			$c = count($comments);
			foreach(var $i = 0; $i < $c, $i++)
			{
				$comments[$i]->answers = $this->getComments($order, $comments[$i]->id, $limit, $offset);
			}
		}
		return $comments;
	}
